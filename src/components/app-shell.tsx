...
export function AppShell({ children }: AppShellProps) {
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_18%_12%,rgba(58,123,213,0.18),transparent_28%),radial-gradient(circle_at_82%_8%,rgba(199,168,102,0.12),transparent_24%),linear-gradient(135deg,var(--bg-950),var(--bg-900)_54%,#03060a)] text-archive-text100">
      <div className="mx-auto flex min-h-screen max-w-[var(--content-max)] flex-col gap-4 px-4 py-4 lg:flex-row lg:px-6 lg:py-6">
        <aside className="rounded-panel border border-[rgba(199,168,102,0.42)] bg-[rgba(8,17,27,0.9)] p-5 shadow-panel backdrop-blur lg:sticky lg:top-6 lg:h-[calc(100vh-48px)] lg:w-[var(--sidebar-w)] lg:shrink-0">
          ...
          <nav aria-label="Navigation principale" className="flex flex-wrap gap-2 lg:flex-col">
            {navigationItems.map((item) => (
              <Link
                className="rounded-chip border border-[rgba(199,168,102,0.28)] bg-[rgba(16,32,51,0.6)] px-4 py-2.5 text-sm font-semibold text-archive-text300 transition hover:border-archive-gold300 hover:bg-[rgba(199,168,102,0.12)] hover:text-archive-gold300 lg:rounded-card"
                href={item.href}
                key={item.href}
              >
                {item.label}
