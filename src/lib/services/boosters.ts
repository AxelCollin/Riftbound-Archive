...
export async function getBoosterOverview(now = new Date()): Promise<BoosterOverviewView> {
  const [record, ledgerQuantity, recentOpenings, cards] = await Promise.all([
    ...
    prisma.card.findMany({
      ...
      select: { id: true, name: true, collectorNumber: true, rarity: true, kind: true, gameplayType: true, hasShowcase: true, set: { select: { code: true } }, translations: { where: { locale: { in: ["fr-FR", "fr"] } }, select: { locale: true, name: true } } },
...
async function validatePulledCards(client: BoosterPrismaClient, pulls: { cardId: string; variant: CardVariant; quantity: number }[]): Promise<void> {
  for (const pull of pulls) {
    const card = await client.card.findUnique({ where: { id: pull.cardId }, select: { id: true, name: true, rarity: true, kind: true, gameplayType: true, hasShowcase: true } });
