...
Phase 7C adds a minimal booster opening entry flow on `/boosters`. The form records the number of boosters opened, whether the counter should be decremented, and an optional note in `BoosterOpening`; when decrementing, pending virtual accrual is materialized first and an `OPENING_DECREMENT` ledger event is written with a negative quantity. The counter can go negative and insufficient counter balance does not block recording. Card-pull detail entry is deferred to a later booster sub-step, so this phase does not create collection transactions, mutate collection entries, add cards automatically, show a post-opening summary, implement rollback, add pricing, provider sync, Electron work, or change deckbuilding behavior.
