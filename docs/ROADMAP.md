...
Phase 7B adds a read-only accumulated booster counter calculation. The service reads the existing `BoosterSettings` row, passes an explicit current time into pure domain logic, counts only complete elapsed day-based intervals since `accrualAnchorAt`, and displays the calculated allowance on `/boosters`. If no settings row exists, the default Phase 7A settings use the request time as a safe anchor so the read-only counter starts at zero. This phase does not create booster openings, decrement counters, write collection transactions, mutate the anchor, add summaries, add rollback, add pricing, add provider sync, or change deckbuilding behavior.
