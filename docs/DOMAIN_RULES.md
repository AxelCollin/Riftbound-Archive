...
Phase 7B calculates the read-only accumulated booster allowance from persisted `BoosterSettings`. Accumulation counts only complete elapsed intervals since `accrualAnchorAt`, uses elapsed milliseconds with UTC timestamps to avoid local-time drift, clamps future timestamps to zero accumulated boosters, and remains integer-only. `boostersPerInterval` is multiplied by the number of complete intervals; `boostersPerInterval = 0` produces zero accumulated boosters. The calculation must receive an explicit `now` from the caller and must not mutate `accrualAnchorAt`. If no settings row exists, Phase 7A defaults are used with a safe current-time anchor so the calculated counter starts at zero.
